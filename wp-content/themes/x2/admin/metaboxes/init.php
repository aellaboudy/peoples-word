 ...
 */
define( 'CMB_META_BOX_URL', apply_filters( 'cmb_meta_box_url', trailingslashit( str_replace( str_replace('\\', '/', WP_CONTENT_DIR), WP_CONTENT_URL, str_replace('\\', '/', dirname( __FILE__ ) ) ) ) ) );
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			switch ( $field['type'] ) {
				...
				case 'checkbox':
					...
					break;
				case 'multicheck':
					echo '<div class="checkboxgroupoptions"><ul>';
					$i = 1;
...
function cmb_scripts( $hook ) {
    ...
    wp_enqueue_script('admin-js-cookie', get_template_directory_uri() . '/admin/js/jquery.cookie-1.4.1.min.js', array('jquery'));
    wp_enqueue_script('admin-js', get_template_directory_uri() . '/admin/js/admin.js', array('admin-js-cookie'));
    wp_localize_script('admin-js', 'admin_params', array(
        'ajax_url'          => site_url('/wp-admin/admin-ajax.php'),
        'blog'              => __('blog', 'x2'),
        'flux_slider'       => __('flux slider', 'x2'),
        'default_slider'    => __('default', 'x2'),
        'reset_all_options' => __( 'Are you sure want reset all settings to default?', 'x2' ),
    ) );
}
...
function cmb_editor_footer_scripts() {
	if ( isset( $_GET['cmb_force_send'] ) && 'true' == $_GET['cmb_force_send'] ) {
		...
		if ( empty( $label ) ) $label = __("Select File", 'x2');
		?>
		<script type="text/javascript">
		jQuery(function($) {
