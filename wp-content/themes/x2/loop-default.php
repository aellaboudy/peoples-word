<?php global $wp_query, $the_lp_query, $cap, $show_pagination;

if(!$the_lp_query)
	$the_lp_query = $wp_query;

if ( $the_lp_query->have_posts() ) : ?>
<div id="featured_posts_default">
	<div id="list_posts_default" class="loop-designer list-posts-all">
		...
		if($show_pagination != 'hide'){
			if( $cap->posts_lists_hide_pagenavi == 'show' ){
			    $x2_tmp .='<div id="_default" class="navigation">';
			...
			if($cap->posts_lists_hide_pagenavi == 'pagenavi'){
			    if(function_exists('wp_pagenavi')){
			        ob_start();
			            ...
			            $x2_tmp_wp_pagenavi = str_replace('class=\'wp-pagenavi\'', 'id="_default" class="wp-pagenavi navigation"', $x2_tmp_wp_pagenavi);
